...
def replace_node_with_nothing(modified_tree, original_tree, node_id):
    ...
    for line_idx in range(node_end-node_start):
        if 'ascendancyName' in modified_tree[node_start]:
            ...
            is_ascendancy = True
        elif 'isMastery' in modified_tree[node_start]:
            is_mastery = True
            #print('Mastery node found at '+str(node_id)+'.\n')
...
'            [\"stats\"] = {},\n']
            ...
            if 'isMastery' not in modified_tree[node_start]:
                print('Replacement mastery node does not match node type')
                is_MismatchedType = True
            is_mastery = True
        modified_tree.pop(node_start)
    if node_found:
